			   ...
			   rpmsg_ns_bind_cb ns_bind_cb)
{
	...
	printf("finished.\nResource table base: 0x%x.\nMinimum reserved memory size: 0x%x.\n",
			(int)SHARED_RSC_TABLE, (SHARED_BUF_OFFSET + SHARED_MEM_SIZE));

	printf("Initializing rpmsg IPI device... ");
...
platform_create_rproc(int proc_index, int rsc_index)
{
	...
	remoteproc_mmap(&rproc_prime, &pa,
				NULL, rsc_size,
