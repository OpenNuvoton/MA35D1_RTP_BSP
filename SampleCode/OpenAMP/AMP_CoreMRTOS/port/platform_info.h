...
struct remoteproc_priv {
	...
	int      reload; // check if the flow is reload at init stage
	uint32_t desc_num;
	uint32_t desc_rxbuf; // size of buf managed by 1 desc
	uint32_t desc_txbuf;
	uint32_t shmem_base;
	uint32_t shmem_tx_base;
	uint32_t shmem_rx_base;
	// tx
...
struct rpmsg_endpoint_info{
	...
	uint32_t type; // EPT_TYPE_TX or EPT_TYPE_RX
	uint32_t size; // Tx: request data length in byte; Rx: reserved
};
