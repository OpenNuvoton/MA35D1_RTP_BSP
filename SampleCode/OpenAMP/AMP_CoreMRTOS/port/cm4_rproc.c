			...
			const struct remoteproc_ops *ops, void *arg)
{
	...
	/* Init IPI here */
	if(RXIPI_BASE->INTENM4 & (1 << RXIPI_CH_SEL))
		rproc_priv.reload = 1;

	if(HWSEM_GET_INT_FLAG(RXIPI_BASE, RXIPI_CH_SEL))
		HWSEM_CLR_INT_FLAG(RXIPI_BASE, RXIPI_CH_SEL);
	HWSEM_ENABLE_INT(RXIPI_BASE, RXIPI_CH_SEL);
	NVIC_EnableIRQ(RXIPI_IRQ_NUM);
...
static void ma35_rproc_remove(struct remoteproc *rproc)
{
	...
	(void)dev;
	HWSEM_DISABLE_INT(RXIPI_BASE, RXIPI_CH_SEL);
	NVIC_DisableIRQ(RXIPI_IRQ_NUM);
...
void HWSEM0_IRQHandler(void)
{
	...
	flag = RXIPI_BASE->INTSTSM4 & ((1 << RXIPI_CH_SEL) | (1 << TXIPI_CH_SEL));
	if(flag)
		RXIPI_BASE->INTSTSM4 = flag;
	else
		return;
	...
	if(rsc_table->reserved[1] == IPI_CMD_REQUEST)
	{
		...
		// send notification to remote in IQR
		HWSEM_LOCK(TXIPI_BASE, TXIPI_CH_SEL, TXIPI_KEY);
		HWSEM_UNLOCK(TXIPI_BASE, TXIPI_CH_SEL, TXIPI_KEY);
		return;
	}
...
// Use IPI to generate INT to remote
static int ma35_notify_remote(struct metal_io_region *io_resion, uint32_t id)
{
	...
	if(!(TXIPI_BASE->INTENA35 & (1 << TXIPI_CH_SEL)))
		return -EACCES; // remote init not ready
	if(TXIPI_BASE->INTSTSA35 & (1 << TXIPI_CH_SEL))
		return -EBUSY; // remote busy

	/* lock and unlock with IRQ disabled for fastest signal */
	__disable_irq();
	ret = HWSEM_Try_Lock(TXIPI_BASE, TXIPI_CH_SEL, TXIPI_KEY);
	if(!ret)
		HWSEM_UNLOCK(TXIPI_BASE, TXIPI_CH_SEL, TXIPI_KEY);
	__enable_irq();
	...
	for(i = 0; i < rproc_priv.desc_num; i++)
	{
		...
		if(!memcmp(ns, rsc_table->desc_vring1[i].ns, NO_NAME_SERVICE))
		{
			...
			if(ma35_allocate_buf_pool(ept_priv, ret))
				...
				printf("Binded with remote ept \"%s\" with size %d.\n", ns, ret);
		    ...
		    struct rpmsg_virtio_shm_pool *shpool)
{
	...
	base = (uint32_t)rvdev->shpool->base;
	...
	if(rproc_priv.reload)
	{
		rproc_priv.reload = 0;
		rsc_table->reserved[0] = IPI_CMD_RELOAD;
		// send notification to remote in IQR
		HWSEM_LOCK(TXIPI_BASE, TXIPI_CH_SEL, TXIPI_KEY);
		HWSEM_UNLOCK(TXIPI_BASE, TXIPI_CH_SEL, TXIPI_KEY);
	}
