...
int MA35D1_OpenAMP_Init(int RPMsgRole, rpmsg_ns_bind_cb ns_bind_cb)
{
    ...
    rpmsg_virtio_init_shm_pool(&shpool, (void *)VRING_BUF_ADDR, (size_t)SHM_SIZE);  // share memory ... 128x2
...
int OPENAMP_send_data(struct rpmsg_endpoint *ept, const void *data, int len)
{
    ...
    memcpy((void *)((char *)SHM_TX_START_ADDRESS), data, u32TX_Length);
    ...
    if (WHC_Send(WHC0, mbox_ch, au32TxBuf) < 0)
        printf("  Send fail ...\n\n");
